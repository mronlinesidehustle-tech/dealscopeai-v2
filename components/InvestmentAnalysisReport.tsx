...
const InvestorFitCard: React.FC<{
  ...
  return (
    <div className={`p-5 rounded-lg border ${bg} ${txt}`}>
      <div className="flex items-center mb-2">
        {icon}
        <h4 className="text-xl font-bold">
          {good ? 'This Deal Fits Investor Criteria' : 'This Deal May Not Fit Criteria'}
        </h4>
      </div>
      <p className="text-sm whitespace-pre-line">{fit.analysis}</p>
    </div>
  );
};
  ...
  <div className="overflow-x-auto">
    <table className="min-w-full bg-white border border-slate-200 text-sm dark:bg-slate-800 dark:border-slate-700">
      <thead className="bg-slate-50 dark:bg-slate-700/50">
        <tr>
          {['Address', 'Sold Date', 'Sold Price', 'SqFt', 'Bed/Bath'].map((h) => (
            <th key={h} className="px-4 py-2 text-left font-semibold text-slate-600 dark:text-slate-300">
              {h}
            </th>
          ))}
        </tr>
      ...
      <tbody className="divide-y divide-slate-200 dark:divide-slate-700">
        {comps.map((c, i) => (
          <tr key={i} className="hover:bg-slate-50 dark:hover:bg-slate-700/50">
            ...
            <td className="px-4 py-2">{c.soldDate}</td>
            <td className="px-4 py-2 font-medium">{c.soldPrice}</td>
            <td className="px-4 py-2">{c.sqft}</td>
            <td className="px-4 py-2">{c.bedBath}</td>
          </tr>
        ))}
      </tbody>
    </table>
...
export const InvestmentAnalysisReport: React.FC<InvestmentAnalysisReportProps> = ({ analysis }) => {
  ...
  const repairBadgeTheme = repairInfo.color
  ...
  return (
    <section className="space-y-8">
      ...
      {/* condition */}
      <div className="bg-white p-6 rounded-xl shadow-lg border border-slate-200 dark:bg-slate-800/50 dark:border-slate-700 space-y-4">
        <h4 className="text-lg font-semibold">Property Condition</h4>
        <p>{analysis.propertyCondition}</p>
        <span className={`px-2 py-1 text-xs font-semibold rounded-full ${repairInfo.color} ${repairBadgeTheme}`}>
          {analysis.estimatedRepairLevel} Rehab
        </span>
        <p className="text-sm">{repairInfo.description}</p>
      </div>

      {/* comps */}
      <div className="bg-white p-6 rounded-xl shadow-lg border border-slate-200 dark:bg-slate-800/50 dark:border-slate-700">
        <h3 className="text-2xl font-bold mb-4">Comparable Sales</h3>
        <CompsTable comps={analysis.comparables} />
      </div>

      {/* exit strategies */}
      <div className="bg-white p-6 rounded-xl shadow-lg border border-slate-200 dark:bg-slate-800/50 dark:border-slate-700">
        <h3 className="text-2xl font-bold mb-4">Exit Strategies</h3>
        <div className="space-y-4">
          {analysis.exitStrategies.map((s: ExitStrategy, i) => (
            <div key={i}>
              <h4 className="font-bold text-sky-700 dark:text-sky-400">{s.strategy}</h4>
      ...
      {/* data sources */}
      {analysis.groundingSources?.length ? (
        <div className="bg-white p-6 rounded-xl shadow-lg border border-slate-200 dark:bg-slate-800/50 dark:border-slate-700">
          <h3 className="text-2xl font-bold mb-4">Data Sources</h3>
          <ul className="list-disc list-inside space-y-1 text-sm">
            {analysis.groundingSources.map((src, i) => (
              <li key={i}>
                <a href={src.uri} target="_blank" rel="noopener noreferrer" className="text-sky-600 hover:underline dark:text-sky-400">
                  {src.title}
                </a>
              </li>
            ))}
